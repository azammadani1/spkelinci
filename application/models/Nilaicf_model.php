...
class Nilaicf_model extends CI_Model {
	...
	public function daftarNilaicf($limit, $start = 0, $q = null){
		...
		return $this->db->select('*, gejala_penyakit.id AS gpid ')
						...
						->order_by('gejala_penyakit.id', 'ASC')
                        ->limit($limit, $start)
						->get()
						->result_array();
	}
	...
	///Md dan Mb dihapus
     function get_gejala_by_penyakit($id,$gejala=null){
         $sql = "select gejala_penyakit.gejala_id, cf_pakar, cf_user from gejala_penyakit where penyakit_id=".$id;
         if($gejala!=null)
